...
class CelebA_HQ(data.Dataset):
    def __init__(self, data_path, attr_path, image_list_path, image_size, mode, selected_attrs):
        ...
        att_list = open(attr_path, 'r', encoding='utf-8').readlines()[1].split()
        atts = [att_list.index(att) + 1 for att in selected_attrs]
        orig_images = np.loadtxt(attr_path, skiprows=2, usecols=[0], dtype=str)
        orig_labels = np.loadtxt(attr_path, skiprows=2, usecols=atts, dtype=int)
        indices = np.loadtxt(image_list_path, skiprows=1, usecols=[1], dtype=int)
